...
def cifar10(datasets_dir='mnist_data/kastner'):
    try:
        import urllib
...
def mnist(datasets_dir='mnist_data/kastner'):
    try:
        import urllib
        urllib.urlretrieve('http://google.com')
    except AttributeError:
...
def lfw(n_imgs=1000, flatten=True, npx=64, datasets_dir='mnist_data/kastner'):
...
if __name__ == "__main__":
    ...
    tr, _, _, = mnist()
    trX, trY = tr
    tf = ConvVAE(image_save_root="mnist_data/kastner",
                 snapshot_file="mnist_data/kastner/mnist_snapshot.pkl")
    trX = floatX(trX)
