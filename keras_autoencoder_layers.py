...
def deconv2d_fast(x, kernel, strides=(1, 1), border_mode='valid', dim_ordering='th',
    ...
    if 0:#_on_gpu() and dnn.dnn_available():
        if border_mode == 'same':
            assert (strides == (1, 1))
...
class Deconvolution2D(Convolution2D):
    ...
    def __init__(self, binded_conv_layer, nb_out_channels=1, *args, **kwargs):
        ...
        kwargs['nb_filter'] = 1#self._binded_conv_layer.nb_filter
    ...
    def build(self, input_shape):
        ...
        # print('self._binded_conv_layer.W.shape: %s' % str(self._binded_conv_layer.W.shape))
        # print('self.dim_ordering: %s' % self.dim_ordering)
    ...
    @property
    def output_shape(self):
        print('output_shape property called')
